...
func (s Service) HandleGetRandomReview(ctx *fiber.Ctx) error {
	unitType := ctx.Query("type", "DEFAULT")

	switch strings.ToUpper(unitType) {
	case "DEFAULT":
		idx := rand.Intn(len(DefaultSimpleReviews))
		unit := DefaultSimpleReviews[idx]
...
func (s Service) HandleGetFastReviewFromExplainLog(ctx *fiber.Ctx) error {
